...
func update_selection_position():
	...
	var tile_pos = basic_tile.local_to_map(basic_tile.to_local(mouse_pos))  # Get tile coordinates
	var world_pos = basic_tile.to_global(basic_tile.map_to_local(tile_pos))  # Convert to world position
